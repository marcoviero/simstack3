...
class SimstackResults(SimstackToolbox):
	...
	def plot_lir_vs_z(self):
		size_data_array = [len(self.config_dict['parameter_names'][i]) for i in
						   self.config_dict['parameter_names']]
		sed_data_array = np.zeros(size_data_array)
		z_data_array = np.zeros(size_data_array[0])
		zlen = 2
		plen = 1
		fig, axs = plt.subplots(plen, zlen, figsize=(36, 10))
		for z, zlab in enumerate(self.results_dict['SED_df']['flux_density']):
			...
			for p, plab in enumerate(self.results_dict['SED_df']['flux_density'][zlab]):
				...
				for im, mlab in enumerate(sed):
					...
					sed_params = self.results_dict['SED_df']['SED'][zlab][plab][mlab]
					z_data_array[z] = zmid
					sed_data_array[z, im, p] = np.log10(LIR)

		for im in range(size_data_array[1]):
			for ip in range(size_data_array[2]):

				axs[ip].scatter(z_data_array, sed_data_array[:, im, ip])
				axs[ip].plot(z_data_array, sed_data_array[:, im, ip])
				if not ip:
